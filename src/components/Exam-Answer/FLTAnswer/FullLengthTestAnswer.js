    ...
    const fetchData = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          // Build a name-based block ordering from the exam sections so
          // answer blocks, correct-answer blocks, AND questions all use the
          // same logical order (e.g. "2.1" < "2.2" < "2.10").
          const readingSectionOrder = reading_set?.Reading && Array.isArray(reading_set.Reading)
            ? [...reading_set.Reading]
                .sort((a, b) => {
                  const nameA = a.exam_name || '';
                  const nameB = b.exam_name || '';
          ...
          if (reading_set?.student_answers?.Reading && reading_set?.correct_answers?.Reading) {
            // Sort blocks by the exam_name-derived order; fall back to block_id
            const sortByReadingOrder = (a, b) => {
              if (readingSectionOrder) {
                const idxA = readingSectionOrder.indexOf(a.block_id);
                const idxB = readingSectionOrder.indexOf(b.block_id);
                if (idxA !== -1 && idxB !== -1) return idxA - idxB;
              }
              return (a.block_id || 0) - (b.block_id || 0);
            };
            const sortedStudentReadingBlocks = [...reading_set.student_answers.Reading].sort(sortByReadingOrder);
            const sortedCorrectReadingBlocks = [...reading_set.correct_answers.Reading].sort(sortByReadingOrder);
            readingStudentAnswers = sortedStudentReadingBlocks.reduce(
              (acc, curr) => acc.concat(
                curr.answers.sort((a, b) => a.question_number - b.question_number)
              ), []
            );
            readingCorrectAnswers = sortedCorrectReadingBlocks.reduce(
              (acc, curr) => acc.concat(
                curr.answers.sort((a, b) => a.question_number - b.question_number)
              ), []
            );
          ...
          if (reading_set?.Reading && Array.isArray(reading_set.Reading)) {
            ...
            // Sort by exam_name using natural string comparison for correct order
          ...
          // Build a name-based block ordering for listening sections
          const listeningSectionOrder = listening_set?.Listening && Array.isArray(listening_set.Listening)
          ...
          if (listening_set?.student_answers?.Listening && listening_set?.correct_answers?.Listening) {
            // Sort blocks by the exam_name-derived order; fall back to block_id
            const sortByListeningOrder = (a, b) => {
              if (listeningSectionOrder) {
                const idxA = listeningSectionOrder.indexOf(a.block_id);
                const idxB = listeningSectionOrder.indexOf(b.block_id);
                if (idxA !== -1 && idxB !== -1) return idxA - idxB;
              }
              return (a.block_id || 0) - (b.block_id || 0);
            };
            const sortedStudentListeningBlocks = [...listening_set.student_answers.Listening].sort(sortByListeningOrder);
            const sortedCorrectListeningBlocks = [...listening_set.correct_answers.Listening].sort(sortByListeningOrder);
            listeningStudentAnswers = sortedStudentListeningBlocks.reduce(
              (acc, curr) => acc.concat(
                curr.answers.sort((a, b) => a.question_number - b.question_number)
              ), []
            );
            listeningCorrectAnswers = sortedCorrectListeningBlocks.reduce(
              (acc, curr) => acc.concat(
                curr.answers.sort((a, b) => a.question_number - b.question_number)
              ), []
            );
